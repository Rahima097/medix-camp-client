...
const Navbar = () => {
  ...
  const navList = (
    <ul className="flex flex-col gap-2 lg:flex-row lg:items-center lg:gap-8 lg:flex-nowrap">
      {navItems.map((item) => (
  ...
  const profileMenu = (
    <Menu placement="bottom-end" offset={10} portal className="menu-portal">
      <MenuHandler>
        <button className="focus:outline-none">
          <Avatar
            variant="circular"
            alt={user?.displayName || "User"}
            className="border-2 border-blue-500 p-0.5 h-10 w-10"
            src={
              user?.photoURL ||
      ...
      <MenuList className="p-2 shadow-xl border-0 w-48 z-[9999]">
      ...
      >
        <MTNavbar className="sticky top-0 z-10 h-max max-w-full rounded-none px-4 py-3 lg:px-8 lg:py-4 bg-transparent shadow-none border-none">
          <div className="flex items-center justify-between text-blue-gray-900">
            ...
            {/* Center: Navigation (lg only) */}
            <div className="flex-1 hidden lg:flex justify-center">
              {navList}
            </div>

            {/* Right: Profile or Buttons */}
                ...
                <>
                  {/* <Link to="/join-us">
                    <Button
                      variant="text"
                      size="sm"
                      className="hidden lg:inline-flex items-center gap-2 text-gray-700 hover:text-blue-600 font-medium"
                    >
                  ...
                  <Link to="/register">
                    <Button
                      size="sm"
                      className="hidden lg:inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-lg hover:shadow-xl transition-all duration-300"
                    >
                      ...
                      <div className="flex flex-col gap-3 mt-4 pt-4 border-t border-gray-200">
                        {/* <Link to="/join-us" className="w-full">
                          <Button
                            ...
                            size="sm"
                            fullWidth
                            className="border-blue-600 text-blue-600 flex items-center justify-center gap-2"
                          >
                            <FaSignInAlt className="h-4 w-4" />
                            Sign In
                          </Button>
                        ...
                        <Link to="/register" className="w-full">
                          <Button
                            fullWidth
                            size="sm"
                            className="bg-gradient-to-r from-blue-600 to-blue-700 shadow-lg flex items-center justify-center gap-2"
                          >
                            <FaUsers className="h-4 w-4" />
                            Join Us
                          </Button>
